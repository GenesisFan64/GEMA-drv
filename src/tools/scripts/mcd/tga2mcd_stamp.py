...
START_MAP    = 1		# Mapping start at VRAM
BLANK_TILE   = 0x7FF		# Blank Tile
...
if has_pal == True:
	output_file = open(FOLDER_NAME+"/"+MAIN_NAME+"_pal.bin","wb")
	d = pal_len
	while d:
		d -= 1
...
if has_img == True:
	...
	IMG_WDTH = img_width
	IMG_MAX_X = img_width>>4		# 16x16 mode
	IMG_MAX_Y = img_height>>4
	MAP_TILE = START_MAP
	art_file = open(FOLDER_NAME+"/"+MAIN_NAME+"_art.bin","wb")
	mstr_mapname = FOLDER_NAME+"/"+MAIN_NAME+"_map.bin"
	...
	while y_loop:
		...
		while x_loop:
			...
			make_cell(CURR_X+1,CURR_Y)
			make_cell(CURR_X+1,CURR_Y+1)
